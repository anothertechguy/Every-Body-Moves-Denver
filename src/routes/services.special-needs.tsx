...
const features = [
  {
    icon: Puzzle,
    title: "Sensory-Aware",
    body: "Predictable structure, calm spaces, and cues that don't overwhelm.",
  },
  {
    icon: HeartHandshake,
    title: "Patient by Design",
    body: "We move at your pace. Every day looks a little different, and that's okay.",
  },
  {
    icon: Smile,
    title: "Confidence First",
    body: "Small wins, celebrated loudly. Movement becomes something to be proud of.",
  },
  {
    icon: Shield,
              ...
              <Reveal>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link to="/contact" className="btn-primary">
                    Schedule a trial session <ArrowRight className="h-4 w-4" />
                  </Link>
            ...
            {features.map((f, i) => (
              <Reveal key={f.title} delay={i * 90}>
                <div className="soft-card p-7 h-full transition-transform hover:-translate-y-2 duration-500">
                  <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-sky to-ink-soft flex items-center justify-center text-ink">
                    <f.icon className="h-5 w-5" />
                  </div>
                  <div className="mt-5 font-display text-xl text-ink">{f.title}</div>
