          ...
          {cards.map((c, i) => (
            <Reveal key={c.to} delay={i * 80}>
              ...
              >
                ...
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sand/70 text-ink text-xs uppercase tracking-wider font-semibold">
                    <c.icon className="h-4 w-4 text-rust" /> {c.tag}
                  </div>
                  <h2 className="mt-4 font-display text-4xl md:text-5xl text-ink">{c.title}</h2>
                  <p className="mt-5 text-lg text-muted-foreground">{c.body}</p>
                  <div className="mt-7 inline-flex items-center gap-2 text-rust font-semibold">
                    Explore {c.title}
